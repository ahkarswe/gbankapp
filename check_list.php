...
<body>
    ...
    <h1 class="text-center text-primary">NOC Daily Check List</h1>
    <br>
    <div class="container mb-1">
        <p class="text-end"><a href="/chkdownload.php">export check List to csv</a></p>
    </div>
  ...
  <div class="row mb-3">
  <div class=" offset-md-1 col-md-5 mb-md-0 mb-sm-3   d-grid">
...
<form  method="POST">
  ...
  <option >U Htet Wai Yan Lwin</option>
                ...
                <button class="btn btn-warning" name="chkbtn">View by Nmae</button>
        </div>
        </div>
        ...
        <div class="input-group mb-3">
        <select class="form-select form-select-sm" name="chkviewArea">
            <option>Select By Checking Area</option>
             <option>DC Room</option>
               <option>Power Room</option>
                <option>Generator</option>
        </select>
                <button class="btn btn-danger" name="chkbtnArea">View by Area</button>
        </div>
        ...
        <input type="date" class="form-control ms-2" id="endDate" name="endDate">
                <button class="btn btn-success" name="chkbtnDate">View by Date</button>
        </div>
            </div>
...
if(isset($_POST["chkbtn"]) ||isset($_POST["chkbtnDate"]) || isset($_POST["chkbtnArea"])){
...
}elseif(isset($_POST["chkbtnDate"])){
    $startDate=$_POST["startDate"];
    $endDate=$_POST["endDate"];
    $sql="SELECT * FROM checklist WHERE date between '$startDate' and '$endDate' ORDER BY date DESC,time DESC";
    $result = $conn->query($sql);
    while($row = $result->fetch_assoc()){
        echo "
        <tr>
...
}elseif(isset($_POST["chkbtnArea"])){
    $areaView=$_POST["chkviewArea"];
    if($areaView=="DC Room"){
        $sql="SELECT * FROM checklist WHERE area = 'DC Room' ORDER BY date DESC,time DESC";
        $result = $conn->query($sql);}
    if($areaView=="Power Room"){
        $sql="SELECT * FROM checklist WHERE area = 'Power Room' ORDER BY date DESC,time DESC";
        $result = $conn->query($sql);}  
        if($areaView=="Generator"){
            $sql="SELECT * FROM checklist WHERE area = 'Generator' ORDER BY date DESC,time DESC";
            $result = $conn->query($sql);}  
