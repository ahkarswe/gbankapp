...
<div class="input-group">
        <select class="form-select form-select-sm" name="chkusecase">
            <option>Select usecase</option>
             <option>Core Infra</option>
               <option>Mobile Banking</option>
                <option>MIS</option>
                <option>7 Mile</option>
                <option>CBM NET 2 MDY</option>
                ...
                <button class="btn btn-success" name="chkbtnusecase">Search</button>
        </div>
        </form>
    </div>
</div>
...
if(isset($_POST['chkbtnusecase'])){
    
    if($_POST["chkusecase"]=="Core Infra"){
        $usecase = "Core Infra";
    }
    if($_POST["chkusecase"]=="Mobile Banking"){
        $usecase = "Mobile Banking";
    }
    if($_POST["chkusecase"]=="MIS"){
        $usecase = "MIS";
    }
    if($_POST["chkusecase"]=="7 Mile"){
        $usecase = "7 Mile Office";
    }
    if($_POST["chkusecase"]=="CBM NET 2 MDY"){
        $usecase = "CBM NET 2 MDY";
    }
    if($_POST["chkusecase"]=="Call Center"){
        $usecase = "Call Center";
    }
    if($_POST["chkusecase"]=="Domain"){
        $usecase = "Domain Name";
    }
    if($_POST["chkusecase"]=="Web Hosting"){
        $usecase = "Web Hosting";
    }
    


    $sql="SELECT * FROM inventory WHERE use_case=\"$usecase\"";
    $result = $conn->query($sql);
    if(!$result){
        die("Invalid query:". $conn->error);
    }
    if($_POST["chkusecase"]=="Select usecase"){
        $sql="SELECT * FROM inventory";
    $result = $conn->query($sql);
    if(!$result){
        die("Invalid query:". $conn->error);
